...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
       ...
       if(!prefix1) prefix = '@';
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new EmbedBuilder().setColor(`DARK_BUT_NOT_BLACK`).setDescription(`Usage : \`${prefix}\`rofficial <user>\``)]})
        }
        ...
        let gRole = await client.data.get(`official_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new EmbedBuilder().setColor(`DARK_BUT_NOT_BLACK`).setDescription(`There is no **official Role** set for **Custom Roles**`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await client.data.get(`official_${message.guild.id}`,null);
        ...
        return message.channel.send({embeds : [new EmbedBuilder().setColor(`DARK_BUT_NOT_BLACK`).setDescription(`SuccessFully removed <@&${gRole}> to ${abc}`)]});
    }
              }