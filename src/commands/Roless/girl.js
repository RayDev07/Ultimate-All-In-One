...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
       ...
       if(!prefix1) prefix = '@';
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new EmbedBuilder().setColor(`DARK_BUT_NOT_BLACK`).setDescription(`Usage : \`${prefix}\`girl <user>\``)]})
        }
        ...
        if(!message.guild.roles.cache.has(gRole)){
            await client.data.set(`girl_${message.guild.id}`,null);
